...
if (!class_exists('Tax_Meta_Class')) :
    ...
    class Tax_Meta_Class {
        ...
        public function __construct($meta_box) {
            ...
            $this->SelfPath = get_template_directory_uri() . '/inc/libs/tax-meta-class';
        ...
        public function insert_images() {
            ...
            foreach ($_POST['attachments'] as $attachment_id => $attachment) {
                ...
                $li .= "<a title='" . esc_html__('Delete this image', 'charitywp') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . esc_url($this->SelfPath) . "/images/delete-16.png' alt='" . esc_attr__('Delete', 'charitywp') . "' /></a>";
        ...
        public function wp_ajax_delete_image() {
            ...
            if ($ok) {
                ...
                echo json_encode(array('message' => esc_html__('Cannot delete file. Something\'s wrong.', 'charitywp')));
                die();
            }
        }
        ...
        <img src="';
                    ...
                    echo '" alt="' . esc_html__('Remove', 'charitywp') . '" title="' . esc_html__('Remove', 'charitywp') . '" id="remove-' . $field['id'] . '"></div>';
                    $c = $c + 1;
                }
            ...
            echo '" alt="' . esc_html__('Add', 'charitywp') . '" title="' . esc_html__('Add', 'charitywp') . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_file($field, $meta) {
            ...
            if (!empty($meta) && count($meta) > 0 && !$this->is_array_empty($meta)) {
                ...
                echo '<div style="margin-bottom: 10px"><strong>' . esc_html__('Uploaded files', 'charitywp') . '</strong></div>';
                echo '<ol class="at-upload">';
                foreach ($meta as $att) {
                    // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
                    echo "<li>" . wp_get_attachment_link($att, '', false, false, ' ') . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . esc_html__('Delete', 'charitywp') . "</a>)</li>";
                }
                echo '</ol>';
            }
      ...
      <strong>" . esc_html__('Upload new files', 'charitywp') . "</strong>\n
    </div>\n";
            echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . esc_html__('Add more files', 'charitywp') . "</a>\n
      </div><!-- End .new-files -->\n";
            echo "</td>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_color($field, $meta) {
            ...
            echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__('Select a color', 'charitywp') . "'/>";
