...
?>
	<div class="test wapper_testimonials <?php echo $style_testimonials; ?> <?php echo $hover_testimonials; ?><?php echo $gutter_pt; ?><?php echo $same_size; ?> <?php echo $style_pagination; ?>">
		<div class="testimonials_column">
			<ul class="content_testimonials">
				<?php
				wp_enqueue_script( 'infinitescroll' );
				while ( have_posts() ) : the_post();

					$image_id  = get_post_thumbnail_id( get_the_ID() );
					$image_url = wp_get_attachment_image( $image_id, $images_size, false, array(
						'alt'   => get_the_title(),
						'title' => get_the_title()
					) );
					...
					if ( $style_testimonials == 'standard' ) {
						...
						echo '<div class="testimonials_standard"><h3><a href="' . esc_url( get_permalink( $post->ID ) ) . '" title="' . esc_attr( get_the_title( $post->ID ) ) . '" >' . get_the_title( $post->ID ) . '</a></h3>';
						echo '<span class="p_line"></span>';
						echo '</div>';
						echo '</div>';
					} else { // classic | gallery
						echo '<div class="testimonials-image">' . $image_url . '<div class="testimonials_hover"><div class="thumb-bg""><div class="mask-content">';
